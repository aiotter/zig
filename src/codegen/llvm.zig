...
pub const FuncGen = struct {
    ...
    fn airIntToFloat(self: *FuncGen, inst: Air.Inst.Index) !?*const llvm.Value {
        ...
        const extended = e: {
            const llvm_ty = self.context.intType(rt_int_bits);
            if (operand_scalar_ty.isSignedInt()) {
                break :e self.builder.buildSExtOrBitCast(operand, llvm_ty, "");
            } else {
                break :e self.builder.buildZExtOrBitCast(operand, llvm_ty, "");
            }
        };
